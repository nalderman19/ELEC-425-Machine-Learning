...
for k = ks
    ...
    [pred] = knn(val_digits, train_digits, train_labels, k);
...
[pred] = knn(test_digits, train_digits, train_labels, best_k);

% ******** step 2. ********
% once you get your prediction, please add one line of code to calculate 
% the accuracy
accFinal = sum(pred == test_labels)/length(test_labels);
...
fprintf("The final accuracy on the test set is %.2f %%, \n which is obtained with k = %d, where k is selected using a validation set.",100*accFinal,best_k)
